...
class DetailTawaranTutupAdmin extends StatefulWidget {
  final Map<String, dynamic> item;

  DetailTawaranTutupAdmin({required this.item});

  @override
  _DetailTawaranTutupAdminState createState() =>
      _DetailTawaranTutupAdminState();
}

class _DetailTawaranTutupAdminState extends State<DetailTawaranTutupAdmin> {
  ...
  Future<void> _updateStatusApproval() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Tawaran gagal dibuka: ${response.reasonPhrase}'),
          ),
        );
      }
    } catch (e) {
                            ...
                            child: Flexible(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                      'Periode Pelaksanaan: ${_item['periode']}'),
                                  SizedBox(
                                    height: 8,
                                  ),
                                  Text(
                                      'Lokasi Pelaksanaan: ${_item['lokasi']}'),
                                ],
                              ),
                            ),
                          ),
