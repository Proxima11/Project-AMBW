...
import 'dart:convert';
import 'package:aplikasi_magang/admin/detailtawarantutup_admin.dart';

class PenawarantutupAdmin extends StatefulWidget {
  const PenawarantutupAdmin({super.key});

  @override
  State<PenawarantutupAdmin> createState() => _PenawarantutupAdminState();
}

class _PenawarantutupAdminState extends State<PenawarantutupAdmin> {
  ...
  Future<void> _fetchFromFirebase() async {
    ...
    data.forEach((key, value) {
      if (value['status_approval'].toString() == '2') {
        loadedItems.add({
          'id': key,
  ...
  void _filterItems(String query) {
    setState(() {
      _filteredItems = _items.where((item) {
        ...
        bool passesStatusFilter = statusProject == '2';
  ...
  void _navigateToDetailScreen(
    ...
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => DetailTawaranTutupAdmin(item: item),
      ),
    );
